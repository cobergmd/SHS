...
static int compute_block(Sha512 *sha) {
...
int sha512_hash(Sha512 *sha) {
  ...
  if (*(const unsigned char *)&endian == 0xff) {
    ...
    sha->msg_digest[3] = endian_reverse64(sha->msg_digest[3]);
    sha->msg_digest[4] = endian_reverse64(sha->msg_digest[4]);
    sha->msg_digest[5] = endian_reverse64(sha->msg_digest[5]);
    sha->msg_digest[6] = endian_reverse64(sha->msg_digest[6]);
    sha->msg_digest[7] = endian_reverse64(sha->msg_digest[7]);
  }
